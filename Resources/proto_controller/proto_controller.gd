...
func _slide_camera_smooth_back_to_origin(delta):
	...
	var move_amout = max(self.velocity.length() * delta, walk_speed/1.5 * delta)
	%CameraSmooth.position.y = move_toward(%CameraSmooth.position.y, 0.0, move_amout)
	_saved_camera_global_pos = %CameraSmooth.global_position
...
func _snap_down_to_stairs_check() -> void:
	...
	var was_on_floor_last_frame = Engine.get_physics_frames() - _last_fram_was_on_floor == 1
...
func _handle_air_physics(delta) -> void:
	...
	self.velocity.x = clamp(self.velocity.x, -capped_speed, capped_speed)
	self.velocity.z = clamp(self.velocity.z, -capped_speed, capped_speed)
